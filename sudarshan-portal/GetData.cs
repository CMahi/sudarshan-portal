...
public class GetData
{
                        ...
                        if (i < dt.Rows.Count)
                        {
                            ...
                            if (dtPO != null)
                            {
                                for (int j = 0; j < dtPO.Rows.Count; j++)
                                {
                                    ...
                                    {
                                        img_Data.Append("<th class='imgBtn'>TR</th><th class='imgBtn'>GRN</th><th class='imgBtn'>QCDD</th><th class='imgBtn'>RJ</th><th class='imgBtn'>BB</th><th class='imgBtn'>PP</th><th class='imgBtn'>ER</th></tr><tbody>");
                                        img_Data.Append("<tr><td><a href='#'><img src='../../images/2.png' title='Transition'/></a></td><td><a href='#'><img src='../../images/2.png' title='GRN Complete'/></a></td><td><a href='#'><img src='../../images/4.png' title='QC-Approved with Deviation & Deduction'/></a></td><td><a href='#'><img src='../../images/1.png' title='Rejected'/></a></td><td><a href='#'><img src='../../images/1.png' title='Bill Booked'/></a></td><td><a href='#'><img src='../../images/1.png' title='Payment Processed'/></a></td>" + early + "</tr>");
                                    }
                                    else
                                    {
    ...
    public string get_Early_Payment_Report_Details(string username, string name, int pageno, int rpp)
    {
        string strData = string.Empty;
        string isdata = string.Empty;
        StringBuilder html = new StringBuilder();
        ...
        {
            html.Append("<table class='table table-bordered table-hover' >");
            html.Append("<thead><tr class='grey center'><th>PO No</th><th>Unique No</th><th>Dispatch Note</th><th>Invoice No</th><th>Invoice Amt</th><th>Dispatch Quantity</th><th>Status</th></tr></thead>");
            html.Append("<tbody>");
            DataTable dtData = (DataTable)ActionController.ExecuteAction("", "User_Plant_Mapping_Report.aspx", "getearlypaymentrpt", ref isdata, username, name);
            if (dtData != null)
            {
                int ddl = rpp;
                int from = (pageno - 1) * ddl;
                ...
                for (int i = from; i < to; i++)
                {
                    ...
                    {
                        string encrypt_Str = crypt.Encryptdata(Convert.ToString(dtData.Rows[i]["PO_Number"]));
                        html.Append("<tr><td><a href='#' role='button' id='anc" + (i + 1) + "' onclick='viewData(" + (i + 1) + ")'>" + Convert.ToString(dtData.Rows[i]["PO_Number"]) + "</a></td><td>" + Convert.ToString(dtData.Rows[i]["Unique_No"]) + "</td><td>" + Convert.ToString(dtData.Rows[i]["request_id"]) + "</td><td>" + Convert.ToString(dtData.Rows[i]["Invoice_No"]) + "</td><td style='text-align: right;'>" + Convert.ToString(dtData.Rows[i]["Invoice_Amount"]) + "</td><td>" + Convert.ToString(dtData.Rows[i]["Dispatch_Quantity"]) + "</td><td>" + Convert.ToString(dtData.Rows[i]["Status"]) + "</td><td style='text-align: right; display:none'><input type='text' id='encrypt_po" + (i + 1) + "' value=" + encrypt_Str + " style='display:none'><input type='text' id='venCode" + (i + 1) + "' value=" + Convert.ToString(dtData.Rows[i]["vendor_code"]) + " style='display:none'></td></tr>");
                    }
                }
            }
