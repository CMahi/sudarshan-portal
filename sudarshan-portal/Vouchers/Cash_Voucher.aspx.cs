...
public partial class Cash_Voucher : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            AjaxPro.Utility.RegisterTypeForAjax(typeof(Cash_Voucher));
            if (!Page.IsPostBack)
            {
    ...
    protected void bindData(string process, string req_no)
    {
        try
        {
            //Page.RegisterStartupScript("onclick", "<script language='javascript'>{alert('" + process + "');}</script>");
            if (process == "ADVANCE REQUEST")
            {
            ...
            if (dsData != null)
            {
                ...
                spn_Location.InnerHtml = Convert.ToString(dsData.Tables[0].Rows[0]["P_LOCATION"]);
                string deviated = Convert.ToString(dsData.Tables[0].Rows[0]["deviated"]);
                spn_Doc_Type.InnerHtml = Convert.ToString(dsData.Tables[0].Rows[0]["DOC_TYPE"]);
                if (deviated == "1")
                {
                ...
                DataTable dtApprover = (DataTable)ActionController.ExecuteAction("", "Bank_Voucher.aspx", "getapprovedperson", ref isData, req_no);
                if (dtApprover != null)
                {
                ...
                if (tot_Amount1 > 10000 && process == "OTHER EXPENSES" && spn_Approver.InnerHtml != "" && spn_Approver.InnerHtml != "NA")
                {
                    tdDev1.Style.Add("display", "normal");
                    tdDev2.Style.Add("display", "normal");
                }
                ...
                sb.Append("<table style='width:100%; font-size:small'>");
                ...
                for (int i = 0; i < dsData.Tables[1].Rows.Count; i++)
                {
                    ...
                    //sb.Append("<td class='clsBorder td2' style='border: 1px solid black;'>01</td>");
