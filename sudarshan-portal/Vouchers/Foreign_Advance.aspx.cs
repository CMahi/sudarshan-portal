...
public partial class Foreign_Advance : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            AjaxPro.Utility.RegisterTypeForAjax(typeof(Foreign_Advance));
            if (!Page.IsPostBack)
            {
                if (Convert.ToString(Request.QueryString["R"]) != "")
                {
                    ...
                    process_name = "ADVANCE REQUEST FOREIGN";
                    //req_no = "FT-201617-35";
    ...
    protected void bindData(string process, string req_no)
    {
        ...
        if (dsData != null)
        {
            ...
            string userid = Convert.ToString(dsData.Tables[0].Rows[0]["emp_ad_id"]);
            ...
            DataTable dtAud = (DataTable)ActionController.ExecuteAction("", "Domestic_Travel_Request_Approval.aspx", "getaudit", ref isData, process_id, instance_id);
            if (dtAud != null && dtAud.Rows.Count > 0)
            {
                int ap = 0;
                int dap = 0;
                for(int a=0;a<dtAud.Rows.Count;a++)
                {
                    if (Convert.ToString(dtAud.Rows[a]["stepname"]) == "FOREIGN ADVANCE REQUEST DEVIATION APPROVAL")
                    {
                        dap = dap + 1;
                    }
                    else if (Convert.ToString(dtAud.Rows[a]["stepname"]) == "FOREIGN ADVANCE REQUEST APPROVAL")
                    {
                        ap = ap + 1;
                    }
                }
            ...
            sb.Append("<tr><td colspan='3'><b>Advance Request Summary : </b></td></tr>");

            sb.Append("<tr style='text-align:center'>");
            sb.Append("<td class='clsBorder td2' style='border: 1px solid black;'>Currency</td>");
            sb.Append("<td class='clsBorder td2' style='border: 1px solid black;'>Total Advance</td>");
            sb.Append("<td class='clsBorder td2' style='border: 1px solid black;'>Currency Amount</td>");
            sb.Append("<td class='clsBorder td2' style='border: 1px solid black;'>Forex Card</td></tr>");
            ...
            //decimal netpay = Convert.ToDecimal(tot_Amount1) - adv_amt;
                ...
                sb.Append("<td class='clsBorder td2' style='border: 1px solid black; text-align:center'>" + Convert.ToString(dsData.Tables[0].Rows[0]["adv_amount"]) + "</td>");    
                sb.Append("<td class='clsBorder td2' style='border: 1px solid black; text-align:center'>" + Convert.ToString(dsData.Tables[0].Rows[0]["CURRENCY_AMOUNT"]) + "</td>");
                sb.Append("<td class='clsBorder td2' style='border: 1px solid black; text-align:center'>" + Convert.ToString(dsData.Tables[0].Rows[0]["FOREX_CARD"]) + "</td>");
            ...
            sb.Append("<td class='clsBorder td1' rowspan='2' style='border: 1px solid black;'>BUSINESS AREA</td><td colspan='2' class='clsBorder td1' style='border: 1px solid black;'>ACCOUNT CODE</td><td class='clsBorder td1' style='border: 1px solid black;'>Currency</td></tr>");
