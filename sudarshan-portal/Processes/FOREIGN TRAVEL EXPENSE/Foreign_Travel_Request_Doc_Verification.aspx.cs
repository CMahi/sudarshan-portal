...
public partial class Foreign_Travel_Request_Doc_Verification : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                AjaxPro.Utility.RegisterTypeForAjax(typeof(Foreign_Travel_Request_Doc_Verification));
                if (!Page.IsPostBack)
                {
    ...
    public static string getDataRows(string jFDate, string jTDate, int country_id, string wiid, string desg)
    {
        ...
        try
        {
            ...
            if (dsData != null)
            {
                ...
                if (Convert.ToString(dsData.Tables[0].Rows[0]["Return_Money"]) == "")
                {
                    sb.Append("<td>0.00</td>");
                }
                else
                {
                    sb.Append("<td>" + Convert.ToString(dsData.Tables[0].Rows[0]["Return_Money"]) + "</td>");
                }
                ...
                if (ddlAction.SelectedIndex > 0)
                {
                    if (ddlAction.SelectedValue == "1")
                    {
                        ...
                        if (release_id != "")
                        {
                            ...
                            DTAP = (DataTable)ActionController.ExecuteAction("", "Foreign_Travel_Request_Approval.aspx", "getaccapprover", ref ref_data, "FOREIGN TRAVEL ACCOUNT APPROVAL", 0, pk_mode_id.Text);
                            //FOREIGN TRAVEL ACCOUNT APPROVAL
                            if (DTAP != null)
                            {
                                if (DTAP.Rows.Count > 0)
                                {
                                    isSaved = (string)ActionController.ExecuteAction("", "Foreign_Travel_Request_Approval.aspx", "update", ref ref_data, txt_pk_id.Text, txt_Username.Text, 1,1);
                                    ...
                                    else
                                    {
                                        ...
                                        if (isCreate)
                                        {
                                            try
                                            {
                                                string auditid = (string)ActionController.ExecuteAction(txt_initiator.Text, "Foreign_Travel_Request.aspx", "insertaudittrail", ref isInserted, txtProcessID.Text, txtInstanceID.Text, step_name.Text, "USER", txt_Username.Text, "SUBMIT", "Send For Payement Process", "0", "0");
                                                string msg = "";
                                                CryptoGraphy crypt = new CryptoGraphy();
                                                string process_name = crypt.Encryptdata("FOREIGN TRAVEL EXPENSE");
                                                string req_no = crypt.Encryptdata(spn_req_no.InnerHtml);
                                                msg = "<pre><span style='font-size: medium;'>Dear Sir/Madam,</span></pre><pre><span style='font-size: medium;'>Foreign Travel Request Has Been Approved Succefully and Sent For Payment Process.</span></pre><pre><span style='font-size: medium;'>Request No: " + spn_req_no.InnerHtml + "</span></pre><pre><span style='font-size: medium;'>Created By: " + span_ename.InnerHtml.Trim() + "</span></pre><pre>INTERNET URL:https://" + compname + "/Sudarshan-Portal/Login.aspx</pre><pre>&nbsp;</pre><pre><span style='color: red; font-size: medium;'><em><strong>This is a system generated message. We request you not to reply to this message.</strong></em></span></pre>";

                                                string emailid = (string)ActionController.ExecuteAction(txt_Username.Text, "Foreign_Travel_Request.aspx", "insetmaildata", ref isInserted, txtProcessID.Text, txtInstanceID.Text, 0, step_name.Text, "SUBMIT", txtApproverEmail.Text, txt_init_mail.Text, msg, "Request No: " + spn_req_no.InnerHtml);
                                            }
                                            catch (Exception)
                                            {
                                                throw;
                                            ...
                                            {
                                                Page.RegisterStartupScript("onclick", "<script language='javascript'>{alert('Foreign Travel Request Has Been Approved Successfully and Sent For Payment Process...!');window.open('../../Portal/SCIL/Home.aspx','frmset_WorkArea');}</script>");
                                            }
                                        }
                                    }//
                                }
