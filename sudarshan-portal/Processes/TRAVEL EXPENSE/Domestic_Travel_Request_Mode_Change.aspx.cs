...
public partial class Domestic_Travel_Request_Mode_Change : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                AjaxPro.Utility.RegisterTypeForAjax(typeof(Domestic_Travel_Request_Mode_Change));
                if (!Page.IsPostBack)
                {
        ...
        try
        {
            ...
            if (dt.Tables[0] != null && dt.Tables[0].Rows.Count > 0)
            {
                for (int Index = 0; Index < dt.Tables[0].Rows.Count; Index++)
                {
                    ...
                    tblHTML.Append("</tr>");
                    advance_amount.Text = Convert.ToString(dt.Tables[0].Rows[Index]["amount"]);
                }
            }
            else
            {
    ...
    public static string get_Journey_Data(string jFDate, string jTDate, int wiid)
    {
        ...
        try
        {
            ...
            if (dtSummary != null || dtSummary.Rows.Count > 0)
            {
                ...
                html.Append("<td align='right'><span id='spn_voucher_amt'>" + Convert.ToString(dtSummary.Rows[0]["voucher_amount"]) + "</span></td>");
                if (tot != nreq)
                {
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                {
                    DTAP = (DataTable)ActionController.ExecuteAction("", "Domestic_Travel_Request_Approval.aspx", "getaccapprover", ref ref_data, "ACCOUNT PAYMENT PERSONNEL", ddlAdv_Location.SelectedValue, ddl_Payment_Mode.SelectedValue);
                }
                else
                {
                    DTAP = (DataTable)ActionController.ExecuteAction("", "Domestic_Travel_Request_Approval.aspx", "getaccapprover", ref ref_data, "ACCOUNT PAYMENT PERSONNEL", 0, ddl_Payment_Mode.SelectedValue);
                }
                if (DTAP != null)
                {
                    if (DTAP.Rows.Count > 0)
                    {
                        isSaved = (string)ActionController.ExecuteAction("", "Domestic_Travel_Request_Approval.aspx", "changepaymode", ref ref_data, txt_pk_id.Text, ddlAdv_Location.SelectedValue, ddl_Payment_Mode.SelectedValue);
                        ...
                        else
                        {
                            ...
                            bool isCreate = (bool)WFE.Action.ReleaseStep(txtProcessID.Text, txtInstanceID.Text, "204", "TRAVEL EXPENSE PAYMENT MODE CHANGE", "SUBMIT", txt_Username.Text, txt_Username.Text.Trim(), "", "", "", "", "", "", "", "", "", "", Dval, spn_req_no.InnerHtml, txtWIID.Text, ref isInserted);
                            if (isCreate)
                            {
                                try
                                {
                                    string auditid = (string)ActionController.ExecuteAction(txt_initiator.Text, "Domestic_Travel_Request.aspx", "insertaudittrail", ref isInserted, txtProcessID.Text, txtInstanceID.Text, "TRAVEL EXPENSE PAYMENT MODE CHANGE", "USER", txt_Username.Text, "SUBMIT", "", "0", "0");
                                    ...
                                    string emailid = (string)ActionController.ExecuteAction(txt_Username.Text, "Domestic_Travel_Request.aspx", "insetmaildata", ref isInserted, txtProcessID.Text, txtInstanceID.Text, 0, "TRAVEL EXPENSE PAYMENT MODE CHANGE", "SUBMIT", txtEmailID.Text, txtApproverEmail.Text, msg, "Request No: " + spn_req_no.InnerHtml);
                                }
                                catch (Exception)
                                {
                                    throw;
                    ...
                    {
                        Page.RegisterStartupScript("onclick", "<script language='javascript'>{alert('Account Payment Approver Not Available For " + ddl_Payment_Mode.SelectedItem.Text + " Payment Mode...!');window.open('../../Portal/SCIL/TaskDetails.aspx','frmset_WorkArea');}</script>");
                    }
                }
                else
                {
