...
public partial class Advance_Request_Approval_Foreign : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                AjaxPro.Utility.RegisterTypeForAjax(typeof(Advance_Request_Approval_Foreign));
                if (!Page.IsPostBack)
                {
    ...
    public void fillData(DataTable dt)
    {
        ...
        double perdaysamt = Convert.ToDouble(dt.Rows[0]["allowed_amount"]) / Convert.ToDouble(dt.Rows[0]["noofdays"]);

        double perdaysamt1 = Convert.ToDouble(dt.Rows[0]["amount"]) / Convert.ToDouble(dt.Rows[0]["noofdays"]);

        txt_perdaysamt.Text = perdaysamt.ToString();
        txt_perdaysamt1.Text = perdaysamt1.ToString();
        ...
        txt_remark_hdr.Text = Convert.ToString(dt.Rows[0]["remark"]);
        sb.Append("</div></div>");
    ...
    private void fillAction()
    {
        try
        {
            string Isdata = string.Empty;
            ListItem li = new ListItem("--Select One--", "");
            DataTable dt = (DataTable)ActionController.ExecuteAction("", "Advance_Request_Approval_Foreign.aspx", "action", ref Isdata);
