...
public partial class Other_Expense_Payment_doa_Approval : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                AjaxPro.Utility.RegisterTypeForAjax(typeof(Other_Expense_Payment_doa_Approval));
                if (!Page.IsPostBack)
                {
                ...
                if (ddlAction.SelectedIndex > 0)
                {
                            ...
                            if (DTAP != null)
                            {
                                if (DTAP.Rows.Count > 0)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        bool isCreate = (bool)WFE.Action.ReleaseStep(txtProcessID.Text, txtInstanceID.Text, "200", "OTHER EXPENSE DEVIATION APPROVAL", "SUBMIT", txt_Username.Text, txt_Username.Text.Trim(), "", "", "", "", "", "", "", "", "", "", Dval, spn_req_no.InnerHtml, txtWIID.Text, ref isInserted);
                                        if (isCreate)
                                        {
                                            try
                                            {
                                                string auditid = (string)ActionController.ExecuteAction(txt_Initiator.Text, "Other_Expenses_Request.aspx", "insertaudittrail", ref isInserted, txtProcessID.Text, txtInstanceID.Text, "OTHER EXPENSE DEVIATION APPROVAL", "USER", txt_Username.Text, "SUBMIT", "Approve", "0", "0");
                                                string msg = "";
                                            ...
                                            {
                                                Page.RegisterStartupScript("onclick", "<script language='javascript'>{alert('Other Expense Request Sent To Payment Approval...!');window.open('../../Portal/SCIL/Home.aspx','frmset_WorkArea');}</script>");
                                            }
                                        }
                                    }//
                                }
                                ...
                                {
                                    Page.RegisterStartupScript("onclick", "<script language='javascript'>{alert('Document Verifier Not Available For Payment Mode : " + ddl_Payment_Mode.InnerHtml + "...!');window.open('../../Portal/SCIL/TaskDetails.aspx','frmset_WorkArea');}</script>");
                                }
                            }
                            else
                            {
                    ...
                    else if (ddlAction.SelectedValue == "2")
                    {
                        ...
                        else
                        {
                            ...
                            bool isCreate = (bool)WFE.Action.ReleaseStep(txtProcessID.Text, txtInstanceID.Text, "201", "OTHER EXPENSE DEVIATION APPROVAL", "SEND-BACK", txt_Username.Text, txt_Initiator.Text.Trim(), txt_Initiator.Text.Trim(), "", "", "", "", "", "", "", "", "", Dval, spn_req_no.InnerHtml, txtWIID.Text, ref isInserted);
                            if (isCreate)
                            {
                                try
                                {
                                    string auditid = (string)ActionController.ExecuteAction(txt_Initiator.Text, "Other_Expenses_Request.aspx", "insertaudittrail", ref isInserted, txtProcessID.Text, txtInstanceID.Text, "OTHER EXPENSE DEVIATION APPROVAL", "USER", txt_Username.Text, "SEND-BACK", txt_approver_Remark.Text, "0", "0");
                                    ...
                                    string emailid = (string)ActionController.ExecuteAction(txt_Username.Text, "Other_Expenses_Request.aspx", "insetmaildata", ref isInserted, txtProcessID.Text, txtInstanceID.Text, 0, "OTHER EXPENSE DEVIATION APPROVAL", "SEND-BACK", Init_Email.Text, "", msg, "Request No: " + spn_req_no.InnerHtml);
                                }
                                catch (Exception)
                                {
                                    throw;
                    ...
                    else if (ddlAction.SelectedValue == "3")
                    {
                        ...
                        else
                        {
                            ...
                            bool isCreate = (bool)WFE.Action.ReleaseStep(txtProcessID.Text, txtInstanceID.Text, "202", "OTHER EXPENSE DEVIATION APPROVAL", "REJECT", txt_Username.Text, txt_Username.Text.Trim(), "", "", "", "", "", "", "", "", "", "", Dval, spn_req_no.InnerHtml, txtWIID.Text, ref isInserted);
                            if (isCreate)
                            {
                                try
                                {
                                    string auditid = (string)ActionController.ExecuteAction(txt_Initiator.Text, "Other_Expenses_Request.aspx", "insertaudittrail", ref isInserted, txtProcessID.Text, txtInstanceID.Text, "OTHER EXPENSE DEVIATION APPROVAL", "USER", txt_Username.Text, "REJECT", txt_approver_Remark.Text, "0", "0");
                                    string msg = "<pre><font size='3'>Dear Sir/Madam,</font></pre><pre><font size='3'>Other Expense Request Has Been Rejected.</font></pre><pre><font size='3'>Request No: " + spn_req_no.InnerHtml + "</font></pre><pre><font size='3'>Created By: " + span_ename.InnerHtml.Trim() + "</font></pre><pre>INTERNET URL:https://" + compname + "/Sudarshan-Portal/Login.aspx</pre><pre><font size='3'  color='red'><i><b>This is a system generated message. We request you not to reply to this message.</b></i></font></pre>";
                                    string emailid = (string)ActionController.ExecuteAction(txt_Username.Text, "Other_Expenses_Request.aspx", "insetmaildata", ref isInserted, txtProcessID.Text, txtInstanceID.Text, 0, "OTHER EXPENSE DEVIATION APPROVAL", "REJECT", Init_Email.Text, "", msg, "Request No: " + spn_req_no.InnerHtml);
                                }
                                catch (Exception)
                                {
                                    throw;
