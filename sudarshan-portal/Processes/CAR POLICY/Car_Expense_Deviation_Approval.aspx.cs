    ...
    protected void btnApprove_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            else
            {
                ...
                {
                    DataTable dtRole = (DataTable)ActionController.ExecuteAction("", "Car_Expense_Approval.aspx", "getdocapprover", ref isInserted, "CAR EXPENSE DOC APPROVAL");
                    if (dtRole != null && dtRole.Rows.Count > 0)
                    {
                        ...
                        string isSaved = (string)ActionController.ExecuteAction("", "Car_Expense_Approval.aspx", "update", ref refData, txtProcessID.Text, txtInstanceID.Text, remark, txt_Username.Text, "2");
                        ...
                        else
                        {
                            ...
                            bool isCreate = (bool)WFE.Action.ReleaseStep(txtProcessID.Text, txtInstanceID.Text, txt_StepId.Text, "CAR EXPENSE DOA APPROVAL", "SUBMIT", "", txt_Username.Text.Trim(), "", "", "", "", "", "", "", "", "", "", Dval, spn_req_no.InnerHtml, txtWIID.Text, ref isInserted);
                            if (isCreate)
                            {
                                try
                                {
                                    ...
                                    string emailid = (string)ActionController.ExecuteAction(txt_Username.Text, "Early_Payment_Request.aspx", "insetmaildata", ref isInserted, txtProcessID.Text, txtInstanceID.Text, 0, "CAR EXPENSE DOA APPROVAL", "SUBMIT", Emailids, "", msg, "Car Expense No: " + spn_req_no.InnerHtml);
                                }
                                catch (Exception)
                                {
                                    throw;
                                ...
                                {
                                    Page.RegisterStartupScript("onclick", "<script language='javascript'>{alert('Car Expense Request has been Approved Successfully and Send For Document Verification...!');window.open('../../Portal/SCIL/Home.aspx','frmset_WorkArea');}</script>");
                                }
                            }
                        }
                    }
                    ...
                    {
                        Page.RegisterStartupScript("onclick", "<script language='javascript'>{alert('Document Approval Not Found...!');}</script>");
                    }
                ...
                else if (ddlAction.SelectedItem.Text == "Send-Back")
                {
                    ...
                    if (dtback != null && dtback.Rows.Count > 0)
                    {
                        ...
                        string isSaved = (string)ActionController.ExecuteAction("", "Car_Expense_Approval.aspx", "update", ref refData, txtProcessID.Text, txtInstanceID.Text, remark, txt_Username.Text, "12");
                        ...
                        else
                        {
                            ...
                            bool isCreate = (bool)WFE.Action.ReleaseStep(txtProcessID.Text, txtInstanceID.Text, "233", "CAR EXPENSE DOA APPROVAL", "SEND-BACK", dtback.Rows[0]["actionbyuser"].ToString(), txt_Username.Text.Trim(), "", "", "", "", "", "", "", "", "", "", Dval, spn_req_no.InnerHtml, txtWIID.Text, ref isInserted);
                            if (isCreate)
                            {
                                try
                                {
                                    ...
                                    string emailid = (string)ActionController.ExecuteAction(txt_Username.Text, "Early_Payment_Request.aspx", "insetmaildata", ref isInserted, txtProcessID.Text, txtInstanceID.Text, 0, "CAR EXPENSE DOA APPROVAL", "SUBMIT", Init_Email.Text, "", msg, "Car Expense No: " + spn_req_no.InnerHtml);
                                }
                                catch (Exception ex)
                                {
                                    // throw;
                ...
                else if (ddlAction.SelectedItem.Text == "Reject")
                {
                    remark = txtRemark.Value;
                    refData = string.Empty;
                    string isSaved = (string)ActionController.ExecuteAction("", "Car_Expense_Approval.aspx", "update", ref refData, txtProcessID.Text, txtInstanceID.Text, remark, txt_Username.Text, "13");
                    if (isSaved == null || refData.Length > 0 || isSaved == "false")
                    {
                        string[] errmsg = refData.Split(':');
                        Page.RegisterStartupScript("onclick", "<script language='javascript'>{alert('" + errmsg[0].ToString() + "');}</script>");
                    ...
                    else
                    {
                        ...
                        bool isCreate = (bool)WFE.Action.ReleaseStep(txtProcessID.Text, txtInstanceID.Text, "234", "CAR EXPENSE DOA APPROVAL", "REJECT", txt_Username.Text, txt_Username.Text.Trim(), "", "", "", "", "", "", "", "", "", "", Dval, spn_req_no.InnerHtml, txtWIID.Text, ref isInserted);
                        if (isCreate)
                        {
                            try
                            {
                                ...
                                string emailid = (string)ActionController.ExecuteAction(txt_Username.Text, "Advance_Request.aspx", "insetmaildata", ref isInserted, txtProcessID.Text, txtInstanceID.Text, 0, "CAR EXPENSE DOA APPROVAL", "REJECT", Init_Email.Text, txtEmailID.Text, msg, "Car Expense No: " + spn_req_no.InnerHtml);
                            }
                            catch (Exception)
                            {
                                throw;
