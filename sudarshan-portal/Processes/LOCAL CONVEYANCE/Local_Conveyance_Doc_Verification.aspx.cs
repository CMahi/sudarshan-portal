...
public partial class Local_Conveyance_Doc_Verification : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                AjaxPro.Utility.RegisterTypeForAjax(typeof(Local_Conveyance_Doc_Verification));
                if (!Page.IsPostBack)
                {
            ...
            if (dsData != null)
            {
               ...
               // txt_tremark.Value = Convert.ToString(dsData.Tables[0].Rows[0]["REMARK"]);
                ...
                if (dtUser.Rows.Count > 0)
                {
                    ...
                    {
                        span_glcode.InnerHtml = dtcode.Rows[0]["SAP_GLCode"].ToString();
                    }  
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            {
                txt_Audit.Text = "LOCAL CONVEYANCE DOCUMENT VERIFICATION";                  
                        if (ddlAction.SelectedValue == "Approve")
                        {
                            ...
                            if (DTAP.Rows.Count > 0)
                            {
                                ...
                                else
                                {
                                    ...
                                    bool isCreate = (bool)WFE.Action.ReleaseStep(txtProcessID.Text, txtInstanceID.Text, "134", "LOCAL CONVEYANCE DOCUMENT VERIFICATION", "SUBMIT", txt_Username.Text, txt_Username.Text.Trim(), "", "", "", "", "", "", "", "", "", "", Dval, spn_req_no.InnerHtml, txtWIID.Text, ref isInserted);
                                    if (isCreate)
                                    {
                                        try
                                        {
                                            // string auditid = (string)ActionController.ExecuteAction(txt_Initiator.Text, "Local_Conveyance.aspx", "insertaudittrail", ref isInserted, txtProcessID.Text, txtInstanceID.Text, "LOCAL CONVEYANCE DOCUMENT VERIFICATION", "USER", txt_Username.Text, "SUBMIT", txt_Remark.Value, "0", "0");
                                            if ((Init_Email.Text != "") && (txtEmailID.Text != ""))
                                            {
                                                string msg = "<pre><font size='3'>Dear Sir/Madam,</font></pre><p/> <pre><font size='3'>Local Conveyance Request has been sent for account payment approval.</font></pre><p/> <pre><font size='3'>Request No: " + spn_req_no.InnerHtml + "</font></pre> <pre><font size='3'>Created By: " + span_ename.InnerHtml.Trim() + "</font></pre></p><pre></b><pre>INTERNET URL:https://" + compname + "/Sudarshan-Portal/Login.aspx</pre><br/><pre>Regards</pre><pre><b>Reporting Admin<b></pre><br/><pre></pre><br/><pre><font size='3'  color='red'><i><b>This is a system generated message. We request you not to reply to this message.</b></i></font></pre>";

                                                string emailid = (string)ActionController.ExecuteAction(txt_Username.Text, "Local_Conveyance.aspx", "insetmaildata", ref isInserted, txtProcessID.Text, txtInstanceID.Text, 0, "LOCAL CONVEYANCE DOCUMENT VERIFICATIONL", "SUBMIT", txtApproverEmail.Text, Init_Email.Text, msg, "Request No: " + spn_req_no.InnerHtml);
                                            }
                                        }
                                        catch (Exception)
                                        {
                                        ...
                                        {
                                            Page.RegisterStartupScript("onclick", "<script language='javascript'>{alert('Local Conveyance Request has been sent for account payment approval...!');window.open('../../Portal/SCIL/Home.aspx','frmset_WorkArea');}</script>");
                                        }
                                    }
                                }//
                            }
                            ...
                            {
                                Page.RegisterStartupScript("onclick", "<script language='javascript'>{alert('Account Payment Approvar Not Available For " + ddlAdv_Location.SelectedItem.Text + " Location...!');window.open('../../Portal/SCIL/TaskDetails.aspx','frmset_WorkArea');}</script>");
                            }
                        ...
                        else if (ddlAction.SelectedValue == "Send-Back")
                        {
                            ...
                            isSaved = (string)ActionController.ExecuteAction("", "Local_Conveyance_Approval.aspx", "update", ref ref_data, txt_pk_id.Text, txt_Document_Xml.Text, txt_Condition.Text);
                            if (isSaved == null || ref_data.Length > 0 || isSaved == "false")
                            {
                                string[] errmsg = ref_data.Split(':');
                                Page.RegisterStartupScript("onclick", "<script language='javascript'>{alert('" + errmsg[0].ToString() + "');}</script>");
                            ...
                            else
                            {
                                ...
                                bool isCreate = (bool)WFE.Action.ReleaseStep(txtProcessID.Text, txtInstanceID.Text, "135", "LOCAL CONVEYANCE DOCUMENT VERIFICATION", "SEND-BACK", txt_Username.Text, txt_Initiator.Text.Trim(), txt_Initiator.Text.Trim(), "", "", "", "", "", "", "", "", "", Dval, spn_req_no.InnerHtml, txtWIID.Text, ref isInserted);
                                if (isCreate)
                                {
                                    try
                                    {
                                        //string auditid = (string)ActionController.ExecuteAction(txt_Initiator.Text, "Local_Conveyance.aspx", "insertaudittrail", ref isInserted, txtProcessID.Text, txtInstanceID.Text, "LOCAL CONVEYANCE DOCUMENT VERIFICATION", "USER", txt_Username.Text, "SEND-BACK", txt_Remark.Value, "0", "0");
                                        if ((Init_Email.Text != "") && (txtEmailID.Text != ""))
                                        {
                                            ...
                                            string emailid = (string)ActionController.ExecuteAction(txt_Username.Text, "Local_Conveyance.aspx", "insetmaildata", ref isInserted, txtProcessID.Text, txtInstanceID.Text, 0, "LOCAL CONVEYANCE DOCUMENT VERIFICATION", "SEND-BACK", Init_Email.Text, txtEmailID.Text, msg, "Request No: " + spn_req_no.InnerHtml);
                                        }
                                    }
                                    catch (Exception)
                                    {
                        ...
                        else if (ddlAction.SelectedValue == "Reject")
                        {
                            ...
                            else
                            {
                                ...
                                bool isCreate = (bool)WFE.Action.ReleaseStep(txtProcessID.Text, txtInstanceID.Text, "136", "LOCAL CONVEYANCE DOCUMENT VERIFICATION", "REJECT", txt_Username.Text, txt_Username.Text.Trim(), "", "", "", "", "", "", "", "", "", "", Dval, spn_req_no.InnerHtml, txtWIID.Text, ref isInserted);
                                if (isCreate)
                                {
                                    try
                                    {
                                        // string auditid = (string)ActionController.ExecuteAction(txt_Initiator.Text, "Local_Conveyance.aspx", "insertaudittrail", ref isInserted, txtProcessID.Text, txtInstanceID.Text, "LOCAL CONVEYANCE DOCUMENT VERIFICATION", "USER", txt_Username.Text, "REJECT", txt_Remark.Value, "0", "0");
                                        if ((Init_Email.Text != "") && (txtEmailID.Text != ""))
                                        {
                                            ...
                                            string emailid = (string)ActionController.ExecuteAction(txt_Username.Text, "Local_Conveyance.aspx", "insetmaildata", ref isInserted, txtProcessID.Text, txtInstanceID.Text, 0, "LOCAL CONVEYANCE DOCUMENT VERIFICATION", "REJECT", Init_Email.Text, txtEmailID.Text, msg, "Request No: " + spn_req_no.InnerHtml);
                                        }
                                    }
                                    catch (Exception)
                                    {
