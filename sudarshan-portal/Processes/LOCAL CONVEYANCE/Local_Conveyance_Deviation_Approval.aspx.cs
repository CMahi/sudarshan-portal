...
public partial class Local_Conveyance_Deviation_Approval : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                AjaxPro.Utility.RegisterTypeForAjax(typeof(Local_Conveyance_Deviation_Approval));
                if (!Page.IsPostBack)
                {
    ...
    private void fillAction()
    {
        try
        {
            ...
            DataTable dt = (DataTable)ActionController.ExecuteAction("", "Local_Conveyance_Approval.aspx", "action", ref Isdata);
    ...
    private void ShowMobileUser()
    {
        try
        {
            ...
            if (DTS.Tables[2].Rows.Count > 0)
            {
                ...
                str.Append("<table id='datatable1'  class='table table-bordered table-hover'> <thead><tr class='grey'><th>#</th><th>Vehicle Type</th><th>Bill No</th><th>From Date</th><th>To Date</th><th>KMs</th><th>Bill Amount</th></tr></thead>");
                str.Append("<tbody>");
                for (int i = 0; i < DTS.Tables[2].Rows.Count; i++)
                {
                    ...
                    str.Append("<td align='left'>" + DTS.Tables[2].Rows[i]["BILL_NO"].ToString() + "</td>");
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            {
                txt_Audit.Text = "LOCAL CONVEYANCE DEVIATION APPROVAL";
                    if (ddlAction.SelectedValue == "Approve")
                    {
                        ...
                        {
                            DTAP = (DataTable)ActionController.ExecuteAction("", "Local_Conveyance_Approval.aspx", "getaccapprover1", ref ref_data, "LOCAL CONVEYANCE DOC VERIFIER", 0, ddl_Payment_Mode.SelectedValue);
                        }
                        if (DTAP != null)
                        {
                            if (DTAP.Rows.Count > 0)
                            {
                                ...
                                else
                                {
                                    ...
                                    bool isCreate = (bool)WFE.Action.ReleaseStep(txtProcessID.Text, txtInstanceID.Text, "164", "LOCAL CONVEYANCE DEVIATION APPROVAL", "SUBMIT", txt_Username.Text, txt_Username.Text.Trim(), "", "", "", "", "", "", "", "", "", "", Dval, spn_req_no.InnerHtml, txtWIID.Text, ref isInserted);
                                    if (isCreate)
                                    {
                                        try
                                        {
                                            //string auditid = (string)ActionController.ExecuteAction(txt_Initiator.Text, "Local_Conveyance.aspx", "insertaudittrail", ref isInserted, txtProcessID.Text, txtInstanceID.Text, "LOCAL CONVEYANCE DEVIATION APPROVAL", "USER", txt_Username.Text, "SUBMIT", txt_Remark.Value, "0", "0");
                                            if ((Init_Email.Text != "") && (txtEmailID.Text != ""))
                                            {
                                                string msg = "<pre><font size='3'>Dear Sir/Madam,</font></pre><p/> <pre><font size='3'>Local Conveyance Request has been sent for document verification.</font></pre><p/> <pre><font size='3'>Request No: " + spn_req_no.InnerHtml + "</font></pre> <pre><font size='3'>Created By: " + txt_Initiator.Text.Trim() + "</font></pre></p><pre></b><pre>INTERNET URL:https://" + compname + "/Sudarshan-Portal/Login.aspx</pre><br/><pre>Regards</pre><pre><b>Reporting Admin<b></pre><br/><pre></pre><br/><pre><font size='3'  color='red'><i><b>This is a system generated message. We request you not to reply to this message.</b></i></font></pre>";

                                                string emailid = (string)ActionController.ExecuteAction(txt_Username.Text, "Local_Conveyance.aspx", "insetmaildata", ref isInserted, txtProcessID.Text, txtInstanceID.Text, 0, "LOCAL CONVEYANCE DEVIATION APPROVAL", "SUBMIT", Init_Email.Text, txtEmailID.Text, msg, "Request No: " + spn_req_no.InnerHtml);
                                            }
                                        }
                                        catch (Exception)
                                        {
                                        ...
                                        {
                                            Page.RegisterStartupScript("onclick", "<script language='javascript'>{alert('Local Conveyance Request has been sent for deviation ...!');window.open('../../Portal/SCIL/Home.aspx','frmset_WorkArea');}</script>");
                                        }
                                    }
                                }
                    ...
                    else if (ddlAction.SelectedValue == "Send-Back")
                    {
                        ...
                        else
                        {
                            ...
                            bool isCreate = (bool)WFE.Action.ReleaseStep(txtProcessID.Text, txtInstanceID.Text, "165", "LOCAL CONVEYANCE DEVIATION APPROVAL", "SEND-BACK", txt_Username.Text, txt_Initiator.Text.Trim(), txt_Initiator.Text.Trim(), "", "", "", "", "", "", "", "", "", Dval, spn_req_no.InnerHtml, txtWIID.Text, ref isInserted);
                            if (isCreate)
                            {
                                try
                                {
                                    /// string auditid = (string)ActionController.ExecuteAction(txt_Initiator.Text, "Local_Conveyance.aspx", "insertaudittrail", ref isInserted, txtProcessID.Text, txtInstanceID.Text, "LOCAL CONVEYANCE DEVIATION APPROVAL", "USER", txt_Username.Text, "SEND-BACK", txt_Remark.Value, "0", "0");
                                    if ((Init_Email.Text != "") && (txtEmailID.Text != ""))
                                    {
                                        ...
                                        string emailid = (string)ActionController.ExecuteAction(txt_Username.Text, "Local_Conveyance.aspx", "insetmaildata", ref isInserted, txtProcessID.Text, txtInstanceID.Text, 0, "LOCAL CONVEYANCE DEVIATION APPROVAL", "SEND-BACK", Init_Email.Text, txtEmailID.Text, msg, "Request No: " + spn_req_no.InnerHtml);
                                    }
                                }
                                catch (Exception)
                                {
                    ...
                    else if (ddlAction.SelectedValue == "Reject")
                    {
                        ...
                        else
                        {
                            ...
                            bool isCreate = (bool)WFE.Action.ReleaseStep(txtProcessID.Text, txtInstanceID.Text, "166", "LOCAL CONVEYANCE DEVIATION APPROVAL", "REJECT", txt_Username.Text, txt_Username.Text.Trim(), "", "", "", "", "", "", "", "", "", "", Dval, spn_req_no.InnerHtml, txtWIID.Text, ref isInserted);
                            if (isCreate)
                            {
                                try
                                {
                                    //string auditid = (string)ActionController.ExecuteAction(txt_Initiator.Text, "Local_Conveyance.aspx", "insertaudittrail", ref isInserted, txtProcessID.Text, txtInstanceID.Text, "LOCAL CONVEYANCE DEVIATION APPROVAL", "USER", txt_Username.Text, "REJECT", txt_Remark.Value, "0", "0");
                                    if ((Init_Email.Text != "") && (txtEmailID.Text != ""))
                                    {
                                        string msg = "<pre><font size='3'>Dear Sir/Madam,</font></pre><p/> <pre><font size='3'>Local Conveyance Request has been Rejected.</font></pre><p/> <pre><font size='3'>Request No: " + spn_req_no.InnerHtml + "</font></pre> <pre><font size='3'>Created By: " + txt_Initiator.Text.Trim() + "</font></pre></p><pre></b><pre>INTERNET URL:https://" + compname + "/Sudarshan-Portal/Login.aspx</pre><br/><pre>Regards</pre><pre><b>Reporting Admin<b></pre><br/><pre></pre><br/><pre><font size='3'  color='red'><i><b>This is a system generated message. We request you not to reply to this message.</b></i></font></pre>";

                                        string emailid = (string)ActionController.ExecuteAction(txt_Username.Text, "Local_Conveyance.aspx", "insetmaildata", ref isInserted, txtProcessID.Text, txtInstanceID.Text, 0, "LOCAL CONVEYANCE DEVIATION APPROVAL", "REJECT", Init_Email.Text, txtEmailID.Text, msg, "Request No: " + spn_req_no.InnerHtml);
                                    }
                                }
                                catch (Exception)
                                {
                                ...
                                {
                                    Page.RegisterStartupScript("onclick", "<script language='javascript'>{alert('Local Conveyance Request has been Rejected...!');window.open('../../Portal/SCIL/Home.aspx','frmset_WorkArea');}</script>");
                                }
                            }
                        }
                    }
