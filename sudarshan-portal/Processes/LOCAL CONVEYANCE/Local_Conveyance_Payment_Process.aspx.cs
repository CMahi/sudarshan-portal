...
public partial class Local_Conveyance_Payment_Process : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                AjaxPro.Utility.RegisterTypeForAjax(typeof(Local_Conveyance_Payment_Process));
                if (!Page.IsPostBack)
                {
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            {
                txt_Audit.Text = "LOCAL CONVEYANCE PAYMENT APPROVAL";               
                     if (ddlAction.SelectedValue == "Approve")
                        {
                                ...
                                else
                                {
                                    ...
                                    bool isCreate = (bool)WFE.Action.ReleaseStep(txtProcessID.Text, txtInstanceID.Text, "137", "LOCAL CONVEYANCE PAYMENT APPROVAL", "SUBMIT", txt_Username.Text, txt_Username.Text.Trim(), "", "", "", "", "", "", "", "", "", "", Dval, spn_req_no.InnerHtml, txtWIID.Text, ref isInserted);
                                    if (isCreate)
                                    {
                                        try
                                        {
                                            // string auditid = (string)ActionController.ExecuteAction(txt_Initiator.Text, "Local_Conveyance.aspx", "insertaudittrail", ref isInserted, txtProcessID.Text, txtInstanceID.Text, "LOCAL CONVEYANCE PAYMENT APPROVAL", "USER", txt_Username.Text, "SUBMIT", txt_Remark.Value, "0", "0");
                                            if ((Init_Email.Text != "") && (txtEmailID.Text != ""))
                                            {
                                                string msg = "<pre><font size='3'>Dear Sir/Madam,</font></pre><p/> <pre><font size='3'>Local Conveyance Request has been Approved.</font></pre><p/> <pre><font size='3'>Request No: " + spn_req_no.InnerHtml + "</font></pre> <pre><font size='3'>Created By: " + span_ename.InnerHtml.Trim() + "</font></pre></p><pre></b><pre>INTERNET URL:https://" + compname + "/Sudarshan-Portal/Login.aspx</pre><br/><pre>Regards</pre><pre><b>Reporting Admin<b></pre><br/><pre></pre><br/><pre><font size='3'  color='red'><i><b>This is a system generated message. We request you not to reply to this message.</b></i></font></pre>";

                                                string emailid = (string)ActionController.ExecuteAction(txt_Username.Text, "Local_Conveyance.aspx", "insetmaildata", ref isInserted, txtProcessID.Text, txtInstanceID.Text, 0, "LOCAL CONVEYANCE PAYMENT APPROVAL", "SUBMIT", Init_Email.Text, txtEmailID.Text, msg, "Request No: " + spn_req_no.InnerHtml);
                                            }
                                        }
                                        catch (Exception)
                                        {
                                        ...
                                        {
                                            Page.RegisterStartupScript("onclick", "<script language='javascript'>{alert('Local Conveyance Request has been Approved...!');window.open('../../Portal/SCIL/Home.aspx','frmset_WorkArea');}</script>");
                                        }
                                    }
                                }                          
                        ...
                        else if (ddlAction.SelectedValue == "Send-Back")
                        {
                           ...
                           isSaved = (string)ActionController.ExecuteAction("", "Local_Conveyance_Approval.aspx", "update", ref ref_data, Convert.ToInt32(txt_Condition.Text), txtProcessID.Text, txtInstanceID.Text, txt_Username.Text, txt_Remark.Value, ddlAction.SelectedItem.Text, txt_Audit.Text);
                            ...
                            else
                            {
                                ...
                                bool isCreate = (bool)WFE.Action.ReleaseStep(txtProcessID.Text, txtInstanceID.Text, "138", "LOCAL CONVEYANCE PAYMENT APPROVAL", "SEND-BACK", txt_Username.Text, txt_Initiator.Text.Trim(), txt_Initiator.Text.Trim(), "", "", "", "", "", "", "", "", "", Dval, spn_req_no.InnerHtml, txtWIID.Text, ref isInserted);
                                if (isCreate)
                                {
                                    try
                                    {
                                        // string auditid = (string)ActionController.ExecuteAction(txt_Initiator.Text, "Local_Conveyance.aspx", "insertaudittrail", ref isInserted, txtProcessID.Text, txtInstanceID.Text, "LOCAL CONVEYANCE PAYMENT APPROVAL", "USER", txt_Username.Text, "SEND-BACK", txt_Remark.Value, "0", "0");
                                        if ((Init_Email.Text != "") && (txtEmailID.Text != ""))
                                        {
                                            ...
                                            string emailid = (string)ActionController.ExecuteAction(txt_Username.Text, "Local_Conveyance.aspx", "insetmaildata", ref isInserted, txtProcessID.Text, txtInstanceID.Text, 0, "LOCAL CONVEYANCE PAYMENT APPROVAL", "SEND-BACK", Init_Email.Text, txtEmailID.Text, msg, "Request No: " + spn_req_no.InnerHtml);
                                        }
                                    }
                                    catch (Exception)
                                    {
 ...
 isSaved = (string)ActionController.ExecuteAction("", "Local_Conveyance_Approval.aspx", "update", ref ref_data, Convert.ToInt32(txt_Condition.Text), txtProcessID.Text, txtInstanceID.Text, txt_Username.Text, txt_Remark.Value, ddlAction.SelectedItem.Text, txt_Audit.Text);
                            ...
                            else
                            {
                                string[] Dval = new string[1];
                                Dval[0] = txt_Username.Text;
                                bool isCreate = (bool)WFE.Action.ReleaseStep(txtProcessID.Text, txtInstanceID.Text, "139", "LOCAL CONVEYANCE PAYMENT APPROVAL", "REJECT", txt_Username.Text, txt_Username.Text.Trim(), "", "", "", "", "", "", "", "", "", "", Dval, spn_req_no.InnerHtml, txtWIID.Text, ref isInserted);
                                if (isCreate)
                                {
                                    try
                                    {
                                        ...
                                        if ((Init_Email.Text != "") && (txtEmailID.Text != ""))
                                        {
                                            ...
                                            string emailid = (string)ActionController.ExecuteAction(txt_Username.Text, "Local_Conveyance.aspx", "insetmaildata", ref isInserted, txtProcessID.Text, txtInstanceID.Text, 0, "LOCAL CONVEYANCE PAYMENT APPROVAL", "REJECT", Init_Email.Text, txtEmailID.Text, msg, "Request No: " + spn_req_no.InnerHtml);
                                        }
                                    }
                                    catch (Exception)
                                    {
                     ...
                     else if (ddlAction.SelectedValue == "Send-Back-Payment")
                     {
                         ...
                         else
                         {
                             string[] Dval = new string[1];
                             Dval[0] = txt_Initiator.Text;
                             bool isCreate = (bool)WFE.Action.ReleaseStep(txtProcessID.Text, txtInstanceID.Text, "209", "LOCAL CONVEYANCE PAYMENT APPROVAL", "SEND-BACK-PAYMENT", txt_Username.Text, txt_Username.Text.Trim(), "", "", "", "", "", "", "", "", "", "", Dval, spn_req_no.InnerHtml, txtWIID.Text, ref isInserted);
                             if (isCreate)
                             {
                                 try
                                 {
                                     ...
                                     if ((Init_Email.Text != "") && (txtEmailID.Text != ""))
                                     {
                                         string msg = "<pre><font size='3'>Dear Sir/Madam,</font></pre><p/> <pre><font size='3'>Local Conveyance Request has been sent for payment mode change....</font></pre><p/> <pre><font size='3'>Request No: " + spn_req_no.InnerHtml + "</font></pre> <pre><font size='3'>Created By: " + span_ename.InnerHtml.Trim() + "</font></pre></p><pre></b><pre>INTERNET URL:https://" + compname + "/Sudarshan-Portal/Login.aspx</pre><br/><pre>Regards</pre><pre><b>Reporting Admin<b></pre><br/><pre></pre><br/><pre><font size='3'  color='red'><i><b>This is a system generated message. We request you not to reply to this message.</b></i></font></pre>";

                                         string emailid = (string)ActionController.ExecuteAction(txt_Username.Text, "Local_Conveyance.aspx", "insetmaildata", ref isInserted, txtProcessID.Text, txtInstanceID.Text, 0, "LOCAL CONVEYANCE PAYMENT APPROVAL", "SEND-BACK-PAYMENT", Init_Email.Text, txtEmailID.Text, msg, "Request No: " + spn_req_no.InnerHtml);
                                     }
                                 }
                                 catch (Exception)
                                 {
                                 ...
                                 {
                                     Page.RegisterStartupScript("onclick", "<script language='javascript'>{alert('Local Conveyance Request has been sent for payment mode change...!');window.open('../../Portal/SCIL/Home.aspx','frmset_WorkArea');}</script>");
                                 }
                             }
                         }
