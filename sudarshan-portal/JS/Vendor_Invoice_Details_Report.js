...
function setSelectedNote(req_note) {
    try {
        Vendor_Invoice_Details_Report.GetCurrentTime(req_note, OnSuccess);
    }
    catch (exception) {
...
function downloadfiles(index) {
    try {
        Vendor_Invoice_Details_Report.GetFileNames(index, OnSuccess1);
    }
    catch (exception) {
...
function gotopage(objButton, str) {
    try {
        ...
        Vendor_Invoice_Details_Report.fillGoToPage1(strData, pgno, str,status, callback_Inco);
    }
    catch (exception) {
...
function searchData() {
    ...
    Vendor_Invoice_Details_Report.fillSearch(str, rpp, status, callback_Inco);
}
...
function change_Image(id,vendor_Code,PO,status)
{
    ...
    if ($("#pk_img" + id + "").val() == "1") {
        ...
        $("#Expand" + id).hide();
         ...
         $("#Expand" + id).show();
        Vendor_Invoice_Details_Report.fillDetail(id,vendor_Code, PO, status, callback_detail);
    }
