...
$(document).ready(function () {
    ...
    var data1 = Foreign_Advance_Request_Modification.getCurrencyData();
    currency_data = data1.value;
    
    $("#advance_Foreign").show();
    var process_id = $("#txtProcessID").val();
    var instance_id = $("#txtInstanceID").val();
    var data = Foreign_Advance_Request_Modification.getLineItems(process_id, instance_id);
    $("#div_line_item").html(data.value);
    ...
    Foreign_Advance_Request_Modification.changetype('2', user, desgid, callback_open_detail);
});
...
function chk_class_To() {
    ...
    //Foreign_Advance_Request_Modification.fillAmountall(desg, tocity, callback_Detail);
}
...
function downloadfiles(index) {
    ...
    window.open('../../Common/FileDownload.aspx?enquiryno='+req_no+'&filename=' + tbl.rows[index].cells[1].innerText + '&filetag=', 'Download', 'left=150,top=100,width=600,height=300,toolbar=no,menubars=no,status=no,scrollbars=yes,resize=no');
}
...
function chk_country_From() {
    ...
    $("#txt_fplace").hide();
    Foreign_Advance_Request_Modification.getCountryFromWiseCityFrom(ddlcntfrm, callback_CountryFrom);
...
function chk_country_To() {
    ...
    $("#txt_tother").hide();
    Foreign_Advance_Request_Modification.getCountryFromWiseCityFrom(ddlcntto, callback_CountryTo);
}

function chk_city_To() {

    var tocountry = $("#ddl_country_to").val();
    var desgid = $("#lbl_Grade").val();
    ...
    //Foreign_Advance_Request_Modification.fillAmountForeign(desgid, tocountry, currency,division, callback_Detail_Foreign);
...
function callback_CountryTo(response) {
    ...
    if ($("#ddl_country_to option:selected").index() > 0) {
        ...
        var currency = Foreign_Advance_Request_Modification.getCountryCurrency(pk_id);
...
function change_Currency(index) {
    ...
    if ($("#base_currency").html() == "INR" && $("#ddlCurrency" + index + " option:selected").text() != "INR") {
        ...
        var spn_inr = Foreign_Advance_Request_Modification.getCurrencyRate(spn_curr);
