...
function searchData() {
    ...
    Early_Payment_Mapping_Report.fillSearch(str, rpp, Username, callback_Inco);
}
...
function gotopage(objButton, str) {
    try {
        ...
        var Username = $("#txt_Username").val();
        Early_Payment_Mapping_Report.fillGoToPage1(strData, pgno, str,Username, callback_Inco);
    }
    catch (exception) {
...
function setSelectedNote(req_note) {
    try {
        Early_Payment_Mapping_Report.GetCurrentTime(req_note, OnSuccess);
    }
    catch (exception) {
...
function downloadfiles(index) {
    try {
        Early_Payment_Mapping_Report.GetFileNames(index, OnSuccess1);
    }
    catch (exception) {
...
function viewVendor(vcode) {
    try {
        ...
        Early_Payment_Mapping_Report.viewVendor(venCode, OnVendor);
    }
    catch (exception) {
