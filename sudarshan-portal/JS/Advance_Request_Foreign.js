...
$(document).ready(function () {
    ...
    var desgid = $("#txt_desg_id").val();
    Advance_Request_Foreign.changetype('2', user, desgid, callback_open_detail);
...
function chk_class_To(i) {
    ...
    Advance_Request_Foreign.fillAmountall(desg, tocity, callback_Detail);
}
...
function checkamount() {
    ...
    var advamt = $('#txt_adamount').val();
    $("#div_advance_mode").show();
    ...
    if (open == 1) {
        ...
        if (parseInt(p_amount) > parseInt(total_amount)) {
            ...
            alert("Amount is greater than Allowed amount...!");
        }
        else {
            $('input[Id="btn_Save"]').removeAttr('disabled', 'disabled');
        }
...
function createxml() {
    try {
        ...
        var rmk = document.getElementById("txt_foreign_remark").value;
        var amt = document.getElementById("txt_foreign_amt").value;
        var alwdamt = document.getElementById("lbl_allowedamt").innerHTML;
        var currency = document.getElementById("ddl_currency").value;
        var curnamt = $("#txt_advance_mode_currency").val();
        var forexamt = $("#txt_advance_mode_forexcard").val();
        ...
        if (Math.round(amt) == totalamt) {
            ...
            $("#txt_advance_mode_forexcard").val('');
            return false;
        }
...
function chk_country_From() {
    ...
    Advance_Request_Foreign.getCountryFromWiseCityFrom(ddlcntfrm, callback_CountryFrom);
...
function chk_country_To() {
    ...
    Advance_Request_Foreign.getCountryFromWiseCityFrom(ddlcntto, callback_CountryTo);
...
function chk_city_To() {
    ...
    Advance_Request_Foreign.fillAmountForeign(desgid, tocountry, currency, callback_Detail_Foreign);
...
function checkamount1() {
    ...
    amt = $("#txt_foreign_amt").val();
    alwdamt = $("#lbl_allowedamt").html();
    curnamt = $("#txt_advance_mode_currency").val();
    forexamt = $("#txt_advance_mode_forexcard").val();
    if (curnamt == "") {
        curnamt = "0";
    }
