...
function searchData() {
    ...
    Pending_Payment_Vendor.fillSearch(str, rpp, callback_Inco);
}
...
function gotopage(objButton, str) {
    try {
        ...
        Pending_Payment_Vendor.fillGoToPage1(strData, pgno, str, callback_Inco);
    }
    catch (exception) {
...
function setSelectedNote(req_note) {
    try {
        Pending_Payment_Vendor.GetCurrentTime(req_note, OnSuccess);
    }
    catch (exception) {
...
function downloadfiles(index) {
    try {
        Pending_Payment_Vendor.GetFileNames(index, OnSuccess1);
    }
    catch (exception) {
...
function viewVendor(vcode) {
    try {
        ...
        Pending_Payment_Vendor.viewVendor(venCode, OnVendor);
    }
    catch (exception) {
