$(document).ready(function () {
    Load_DatePicker();
    allowOnlyNumbers();
    var data = Other_Expenses_Request.dropDownData();
    $("#ddlExp_Head1").html(data.value);
});
        ...
        if (check_Controls()==true) {
            ...
            if ($("#dev_total").val() == "1") {
                ...
                for (var q = 0; q < lastRow; q++) {
                    ...
                    xmldata += "<ROW>";
                    xmldata += "<fk_other_expense_Hdr_Id>#</fk_other_expense_Hdr_Id>";
...
function addToClientTable(name, size) {
    ...
    //var uploadText = $("#FileUpload1").get_inputFile();
    ////var fileInputElement = sender.get_inputFile();
    //uploadText.value = "";
}
...
function changeGL(index)
{
    ...
    if (pk_id == 0) {
        ...
        var glcode = Other_Expenses_Request.getGLCode(pk_id);
