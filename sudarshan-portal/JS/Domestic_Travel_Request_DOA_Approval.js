...
function getSelectDate(fromDate, toDate) {
    try {
        if (fromDate != "" && toDate != "") {
            ...
            Domestic_Travel_Request_DOA_Approval.get_Journey_Data(fromDate, toDate, wiid, AjaxSucceeded);
        }
    }
    catch (ex) {
        // $('#divIns').html("");
...
function get_exp_data(index) {
    try {
        ...
        Domestic_Travel_Request_DOA_Approval.get_Expense_Data(desg, j_val, travel_mode_id, travel_class_id, fplant_id, tplant_id, pk_city, index, division, ExpenseSucceeded);
    }
    catch (ex) {
        alert(ex);
    }
...
function get_dev_policy(index) {
    try {
        ...
        Domestic_Travel_Request_DOA_Approval.get_dev_policy(desg, j_val, travel_mode_id, travel_class_id, DeviationSucceeded);
    }
    catch (ex) {
        alert(ex);
    }
                ...
                if ($("#span_Approver").html() == "NA" || $("#span_Approver").html() == "") {
                    ...
                    if (len > 0) {
                        ...
                        for (var j = 1; j <= len; j++) {
                            ...
                            if (journeyType == "---SELECT ONE---") {
                                alert('Select Journey Type For ' + span_val);
                                return false;
                            }
                            else {
                                if ($("#remark_note" + j).val() == "") {
                                    alert("Select Particular Note For " + span_val);
                                    return false;
                                }
                                ...
                                if (journeyType == "OUTSIDE PLANT" || journeyType == "ONE DAY OUTSTATION") {
                                    ...
                                    if (Travel_Mode == "---SELECT ONE---") {
                                        alert('Select Travel Mode For ' + span_val);
                                        return false;
                                    }
                                    else if (Travel_Class == "---SELECT ONE---") {
                                        alert('Select Travel Class For ' + span_val);
                                        return false;
                                    }
                                    else if (journeyType == "OUTSIDE PLANT" && $("#roomType" + j + " option:selected").text().toUpperCase() == "---SELECT ONE---") {
                                        alert('Select Room Type For ' + span_val);
                                        return false;
                                    }
                                    else if (journeyType == "OUTSIDE PLANT" && $("#hotel_name" + j).val() == "") {
                                        alert('Select Hotel Name For ' + span_val);
                                        return false;
                                    }
                                    else if (journeyType == "OUTSIDE PLANT" && $("#hotel_no" + j).val() == "") {
                                        alert('Select Hotel Contact No. For ' + span_val);
                                        return false;
                                    }
                                ...
                                if (journeyType == "OVERNIGHT STAY WITHIN PLANT" || journeyType == "ONE DAY OUTSTATION WITHIN PLANT") {
                                    ...
                                    if (From_Plant == "---SELECT ONE---") {
                                        alert('Select Source Plant For ' + span_val);
                                        return false;
                                    }
                                    if (journeyType == "ONE DAY OUTSTATION WITHIN PLANT") {
                                        if (To_Plant == "---SELECT ONE---") {
                                            alert('Select Destination Plant For ' + span_val);
                                            return false;
                                        }
                                    }
                                }
...
function check_Final() {
    try {
        ...
        for (var j = 1; j <= jr.length; j++) {
            ...
            else if ($("#hotel_name" + j).val() == "") {
                alert("Enter Hotel Name For " + span_val);
                return false;
            }
            else if ($("#hotel_no" + j).val() == "") {
                alert("Enter Contact Number For " + span_val);
                return false;
            }
            else if ($("#hotel_amount" + j).val() == "") {
                alert("Enter Hotel Charge For " + span_val);
                return false;
            }
        ...
        //view_Progressbar();
        Domestic_Travel_Request_DOA_Approval.get_Final_Data(fromDate, toDate, jr.length, jt_data, concat_data, jt_val, jt_amt, to_city, desg, travel_class_id, plant_id,division, mainSucceeded);
...
function mainSucceeded(response) {
    ...
    if ($("#pre_data2").val() != response.value) {
        document.getElementById("div_finaldetails").innerHTML = response.value;
        $("#pre_data2").val(response.value);
    }
    //$('#divIns').html("");
    calculate_Amount();
}
...
function change_flag(exp, index, i) {
    try {
        ...
        Domestic_Travel_Request_Approval.chagable_or_not("Domestic Travel Expenses", fk_id, data, changeText);
    }
    catch (ex) {
        alert(ex);
    }
...
function chk_class_To(i) {
    ...
    if ($("#To_City" + i + " option:selected").text() == "Other") {
        ...
        Domestic_Travel_Request_DOA_Approval.get_Travel_Class(pk_city, i, ClassSucceeded);
    }
}
...
function get_travel_class(rno) {
    ...
    Domestic_Travel_Request_DOA_Approval.get_travel_class(Travel_Mode, rno, OnTravelClass);
}
...
function show_record() {
    ...
    var desg = $("#txt_designation").val();
    var pgno = $("pageno").val();
    Domestic_Travel_Request_DOA_Approval.showall(search_data, 1, ddl, desg,division, OnshowAll);
}
...
function gotopage(objButton, str) {
    try {
        ...
        var desg = $("#txt_designation").val();
        Domestic_Travel_Request_DOA_Approval.showall(strData, pgno, str, desg,division, OnshowAll);
    }
    catch (exception) {
