...
function Bind_Documents(req_no, process_id) {
    Rfc_Status_Report.fillDocumentDetails(req_no, process_id, callback_Bind);
}
...
function gotopage1(objButton, str) {
    try {
        ...
        Rfc_Status_Report.get_BulkRequests(strData, pgno, str, status, f_date, t_date, OngetData);
    }
    catch (exception) {
...
function get_data() {
    ...
    var f_date = $("#txt_f_date").val();
    var t_date = $("#txt_t_date").val();
    Rfc_Status_Report.get_BulkRequests(search_data, pgno, ddl, status, f_date, t_date, OngetData);
}
$(document).ready(function () {
    //get_data();
...
function getRequestDetails(index) {
    ...
    Rfc_Status_Report.getDetails(req_no, pname, callback_BindRequestData);
}
