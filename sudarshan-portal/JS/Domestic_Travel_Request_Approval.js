...
function getSelectDate(fromDate, toDate) {
    try {
        if (fromDate != "" && toDate != "") {
            ...
            Domestic_Travel_Request_Approval.get_Journey_Data(fromDate, toDate, wiid, AjaxSucceeded);
        }
    }
    catch (ex) {
        // $('#divIns').html("");
...
function get_exp_data(index) {
    try {
        ...
        Domestic_Travel_Request_Approval.get_Expense_Data(desg, j_val, travel_mode_id, travel_class_id, fplant_id, tplant_id, pk_city, index,division, ExpenseSucceeded);
    }
    catch (ex) {
        alert(ex);
    }
...
function get_dev_policy(index) {
    try {
        ...
        Domestic_Travel_Request_Approval.get_dev_policy(desg, j_val, travel_mode_id, travel_class_id, DeviationSucceeded);
    }
    catch (ex) {
        alert(ex);
    }
        ...
        if ($("#empno").html() == "") {
            ...
            if ($("#ddlAction option:selected").index() > 0) {
                if ($("#ddlAction option:selected").index() == 1) {
                    if ($("#dev_travel_class").val() == 1 || $("#dev_policy_amt").val() == 1 || $("#dev_supp_amt").val() == 1) {
                        if ($("#doa_user").val() == "NA" || $("#doa_user").val() == "") {
                            alert("Deviation Approver Not Found. Unable To Proceed Request Further...!");
                            return false;
                        }
                    }
                }
...
function chk_class_To(i) {
    ...
    if ($("#To_City" + i + " option:selected").text() == "Other") {
        ...
        Domestic_Travel_Request_Approval.get_Travel_Class(pk_city, i, ClassSucceeded);
    }
}
...
function get_travel_class(rno) {
    ...
    Domestic_Travel_Request_Approval.get_travel_class(Travel_Mode, rno, OnTravelClass);
}
...
function show_record() {
    ...
    var desg = $("#txt_designation").val();
    var pgno = $("pageno").val();
    Domestic_Travel_Request_Approval.showall(search_data, 1, ddl, desg,division, OnshowAll);
}
...
function gotopage(objButton, str) {
    try {
        ...
        var desg = $("#txt_designation").val();
        Domestic_Travel_Request_Approval.showall(strData, pgno, str, desg, division, OnshowAll);
    }
    catch (exception) {
