...
function getSelectDate(fromDate, toDate) {
    try {
        if (fromDate != "" && toDate != "") {
            ...
            Domestic_Travel_Request_Payment_Process.get_Journey_Data(fromDate, toDate, wiid, AjaxSucceeded);
        }
    }
    catch (ex) {
        // $('#divIns').html("");
...
function get_exp_data(index) {
    try {
        ...
        Domestic_Travel_Request_Payment_Process.get_Expense_Data(desg, j_val, travel_mode_id, travel_class_id, fplant_id, tplant_id, pk_city, index,division, ExpenseSucceeded);
    }
    catch (ex) {
        alert(ex);
    }
...
function get_dev_policy(index) {
    try {
        ...
        Domestic_Travel_Request_Payment_Process.get_dev_policy(desg, j_val, travel_mode_id, travel_class_id, DeviationSucceeded);
    }
    catch (ex) {
        alert(ex);
    }
...
function change_flag(exp, index, i) {
    try {
        ...
        Domestic_Travel_Request_Payment_Process.chagable_or_not("Domestic Travel Expenses", fk_id, data, changeText);
    }
    catch (ex) {
        alert(ex);
    }
...
function chk_class_To(i) {
    ...
    if ($("#To_City" + i + " option:selected").text() == "Other") {
        ...
        Domestic_Travel_Request_Payment_Process.get_Travel_Class(pk_city, i, ClassSucceeded);
    }
}
...
function get_travel_class(rno) {
    ...
    Domestic_Travel_Request_Payment_Process.get_travel_class(Travel_Mode, rno, OnTravelClass);
}
...
function show_record() {
    ...
    var desg = $("#txt_designation").val();
    var pgno = $("pageno").val();
    Domestic_Travel_Request_Payment_Process.showall(search_data, 1, ddl, desg,division, OnshowAll);
}
...
function gotopage(objButton, str) {
    try {
        ...
        var desg = $("#txt_designation").val();
        Domestic_Travel_Request_Payment_Process.showall(strData, pgno, str, desg,division, OnshowAll);
    }
    catch (exception) {
