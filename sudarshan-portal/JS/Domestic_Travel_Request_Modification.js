...
function check_PreDate(fromDate, toDate) {
    ...
    var chk_dates = Domestic_Travel_Request_Modification.check_Dates(fromDate, toDate,pk_id);
...
function getSelectDate(fromDate, toDate) {
    try {
        ...
        if ($("#preFDate").val() == "" && $("#preTDate").val() == "") {
            ...
            Domestic_Travel_Request_Modification.get_Journey_Data(fromDate, toDate, wiid, desg,ofdate,otdate,division, AjaxSucceeded);
        }
        ...
        Domestic_Travel_Request_Modification.get_Journey_Data(fromDate, toDate, wiid, desg,ofdate,otdate,division, AjaxSucceeded);
            ...
            if (diff_date1 <= diff_date2) {
                while (firstDate <= secondDate) {
                    ...
                    if (index > diff_date1) {
                        var ret_data = Domestic_Travel_Request_Modification.row_data(index, firstDate, fromDate, toDate);
        ...
        if (fromDate != "" && toDate != "") {
            ...
            Domestic_Travel_Request_Modification.get_Journey_Data(fromDate, toDate, wiid, desg,ofdate,otdate, AjaxSucceeded);
...
function travelAdd(tableBody) {
    ...
    check_journey_Type1();
    $("#divIns").hide();
    var chkbox = $("#txt_chkbox").val().split("@@");
    if (chkbox[1] == "1") {
        $("#chk_reach_" + chkbox[0]).prop("checked", true);
    }
}
    ...
    for (var index = 1; index <= len; index++) {
       ...
       // $("#div_Travel_Class" + index).val(0);
        ...
        if (journeyType == "---SELECT ONE---") {
            ...
            $("#div_Travel_Mode" + index).show();
            //if ($("#div_Travel_Mode" + index).val() > 0) {
            //    $("#div_Travel_Class" + index).show();
            //}
            //else {
            //    $("#div_Travel_Class" + index).hide();
...
function check_journey_Type(index, cnt) {
    ...
    if (journeyType == "---SELECT ONE---") {
        ...
        $("#div_Travel_Mode" + index).show();
        //if ($("#div_Travel_Mode" + index).val() < 1) {
        //    $("#div_Travel_Class" + index).hide();
        //}
        //else {
        //    $("#div_Travel_Class" + index).show();
...
function get_exp_data(index) {
    try {
        ...
        if ($("#chk_reach_" + index).is(':checked'))
            chkbox = 1;
        Domestic_Travel_Request_Modification.get_Expense_Data(desg, j_val, travel_mode_id, travel_class_id, fplant_id, tplant_id, pk_city, index, chkbox,division, ExpenseSucceeded);
    }
    catch (ex) {
        alert(ex);
    }
...
function get_dev_policy(index) {
    try {
        ...
        Domestic_Travel_Request_Modification.get_dev_policy(desg, j_val, travel_mode_id, travel_class_id, DeviationSucceeded);
    }
    catch (ex) {
        alert(ex);
    }
                    ...
                    if (len > 0) {
                        ...
                        for (var j = 1; j <= len; j++) {
                            ...
                            if (journeyType == "---SELECT ONE---") {
                                ...
                                var tfcity = "";
                                var ttcity = "";
                                if ($("#txt_f_city" + j).val() != "" && $("#txt_f_city" + j).val() != null && $("#txt_f_city" + j).val() != undefined) {
                                    tfcity = $("#txt_f_city" + j).val().toUpperCase();
                                }
                                if ($("#txt_t_city" + j).val() != "" && $("#txt_t_city" + j).val() != null && $("#txt_t_city" + j).val() != undefined) {
                                    ttcity = $("#txt_t_city" + j).val().toUpperCase();
                                }
        ...
        if ($("#empno").html() == "") {
            ...
            if ($("#ddl_Payment_Mode option:selected").index() == 0) {
                    ...
                    if ($("#Final_Amtt").html() != undefined && $("#Final_Amtt").html() != "" && parseFloat($("#Final_Amtt").html()) > 0) {
                        ...
                        for (var rad = 1; rad <= radio.length; rad++) {
                            if (radio[rad - 1].checked) {
                                ...
                                $("#spn_adv_amt").html(advance_amount);
                           }
                        }
                        $("#div_Load").show();
                ...
                //        check_Policy_Deviate();
                        ...
                        var xmldoc = "<ROWSET>";
                        var xml_exp = "<ROWSET>";
                        var pk_voucher_id = $("#txt_pk_id").val();
                        for (var i = 1; i <= tbl.length; i++) {
                            ...
                            //var dv = Domestic_Travel_Request_Modification.get_dev_policy($("#txt_designation").val(), $("#journey_Type" + i).val(), $("#Travel_Mode" + i).val(), $("#Travel_Class" + i).val());
                            //if (dv.value == 1) {
                            ...
                            xmldoc += "<ROW>";
                            xmldoc += "<fk_travel_expense_Hdr_Id>" + pk_voucher_id + "</fk_travel_expense_Hdr_Id>";
                            ...
                            if ($("#chk_reach_" + i).is(':checked')) {
                                xmldoc += "<beyond_time>1</beyond_time>";
                            }
                            else {
                                xmldoc += "<beyond_time>0</beyond_time>";
                            }

                            if ($("#chk_guest_" + i).is(':checked') == true) {
                            ...
                            for (var eh = 1; eh <= exp.length; eh++) {
                                ...
                                if ($("#expenses" + i + "" + eh).val() != undefined) {
                                    xml_exp += "<ROW>";
                                    xml_exp += "<fk_travel_expense_Hdr_Id>" + pk_voucher_id + "</fk_travel_expense_Hdr_Id>";
...
function deviation_Alert() {
    ...
    if (dev_string != "") {
        ...
        //dev_string = "Request Submitted Under Deviation...!";
...
function change_flag(exp, index, i) {
    try {
        ...
        Domestic_Travel_Request_Modification.chagable_or_not("Domestic Travel Expenses", fk_id, data, changeText);
    }
    catch (ex) {
        alert(ex);
    }
            ...
            if (d_allow == 0) {
                if (exp_amt != undefined && exp_amt != "") {
                    if (h_exp_amt != "") {
                        ...
                        //if (parseFloat(exp_amt) > parseFloat(h_exp_amt)) {
                        //    $("#dev_policy_amt").val(1);
                        //}
...
function chk_class_To(i) {
    ...
    if ($("#To_City" + i + " option:selected").text() == "Other") {
        ...
        Domestic_Travel_Request_Modification.get_Travel_Class1(pk_city, i, ClassSucceeded);
...
function get_travel_class(rno) {
    ...
    Domestic_Travel_Request_Modification.get_travel_class(Travel_Mode, rno, OnTravelClass);
}
...
function show_record() {
    ...
    var desg = $("#txt_designation").val();
    var pgno = $("pageno").val();
    Domestic_Travel_Request_Modification.showall(search_data, 1, ddl, desg,division, OnshowAll);
}
...
function gotopage(objButton, str) {
    try {
        ...
        var desg = $("#txt_designation").val();
        Domestic_Travel_Request_Modification.showall(strData, pgno, str, desg, division, OnshowAll);
    }
    catch (exception) {
    ...
    try {
        ...
        if ($("#empno").html() == "") {
            ...
            if ($("#ddl_Payment_Mode option:selected").index() == 0) {
                ...
                //else {
                var chk_dates = Domestic_Travel_Request_Modification.check_Dates($("#travelFromDate").val(), $("#travelToDate").val());
                if (chk_dates.value == "1") {
                    alert("Expense Already Claims Between From and To Dates");
                    return false;
                }
                ...
                for (var i = 1; i <= tbl.length; i++) {
                    ...
                    fdate = getFormatedDate1(firstDate);
                    xmldoc += "<ROW>";
                    xmldoc += "<fk_travel_expense_Hdr_Id>" + $("#txt_pk_id").val() + "</fk_travel_expense_Hdr_Id>";
                    xmldoc += "<travel_date>" + fdate + "</travel_date>";
                    if ($("#journey_Type" + i).val() != undefined) {
                        xmldoc += "<journey_type>" + $("#journey_Type" + i).val() + "</journey_type>";
                    }
                    else {
                        xmldoc += "<journey_type>0</journey_type>";
                    }
                    if ($("#Travel_Mode" + i).val() != undefined) {
                        xmldoc += "<travel_mode>" + $("#Travel_Mode" + i).val() + "</travel_mode>";
                    }
                    else {
                        xmldoc += "<travel_mode>0</travel_mode>";
                    }
                    if ($("#Travel_Class" + i).val() != undefined) {
                        xmldoc += "<travel_class>" + $("#Travel_Class" + i).val() + "</travel_class>";
                    }
                    else {
                        xmldoc += "<travel_class>0</travel_class>";
                    }
                    if ($("#From_Plant" + i).val() != undefined) {
                        xmldoc += "<plant_from>" + $("#From_Plant" + i).val() + "</plant_from>";
                    }
                    else {
                        xmldoc += "<plant_from>0</plant_from>";
                    }
                    if ($("#To_Plant" + i).val() != undefined) {
                        xmldoc += "<plant_to>" + $("#To_Plant" + i).val() + "</plant_to>";
                    }
                    else {
                        xmldoc += "<plant_to>0</plant_to>";
                    }
                    if ($("#From_City" + i).val() != undefined) {
                        xmldoc += "<From_city>" + $("#From_City" + i).val() + "</From_city>";
                    }
                    else {
                        xmldoc += "<From_city>0</From_city>";
                    }
                    if ($("#To_City" + i).val() != undefined) {
                        xmldoc += "<to_city>" + $("#To_City" + i).val() + "</to_city>";
                    }
                    else {
                        xmldoc += "<to_city>0</to_city>";
                    }
                    if ($("#placeclass" + i).val() != undefined) {
                        xmldoc += "<place_class>" + $("#placeclass" + i).text() + "</place_class>";
                    }
                    else {
                        xmldoc += "<place_class>NA</place_class>";
                    }
                    if ($("#roomType" + i).val() != undefined) {
                        xmldoc += "<Room_type>" + $("#roomType" + i).val() + "</Room_type>";
                    }
                    else {
                        xmldoc += "<Room_type></Room_type>";
                    }
                    ...
                    if ($("#txt_f_city" + i).val() != undefined) {
                        xmldoc += "<Other_F_City>" + $("#txt_f_city" + i).val() + "</Other_F_City>";
                    }
                    else {
                        xmldoc += "<Other_F_City></Other_F_City>";
                    }
                    if ($("#txt_t_city" + i).val() != undefined) {
                        xmldoc += "<Other_T_City>" + $("#txt_t_city" + i).val() + "</Other_T_City>";
                    }
                    else {
                        xmldoc += "<Other_T_City></Other_T_City>";
                    }
                    if ($("#remark_note" + i).val() != undefined) {
                        xmldoc += "<remark_note>" + $("#remark_note" + i).val() + "</remark_note>";
                    }
                    else {
                        xmldoc += "<remark_note></remark_note>";
                    }

                    var chkbox = 0;
                    if ($("#chk_reach_" + i).is(':checked')==true){
                        xmldoc += "<beyond_time>1</beyond_time>";
                    }
                    else {
                        xmldoc += "<beyond_time>0</beyond_time>";
                    ...
                    for (var eh = 1; eh <= exp.length; eh++) {
                        ...
                        if ($("#expenses" + i + "" + eh).val() != undefined) {
                            xml_exp += "<ROW>";
                            xml_exp += "<fk_travel_expense_Hdr_Id>" + $("#txt_pk_id").val() + "</fk_travel_expense_Hdr_Id>";
                            xml_exp += "<fk_travel_date>" + fdate + "</fk_travel_date>";
                            xml_exp += "<fk_expense_head_id>" + $("#expenses" + i + "" + eh).val() + "</fk_expense_head_id>";
                            var exp_amt = $("#" + e_name + "_" + i + "_" + eh).val();
...
function incidental_Base(index)
{
	...
	var fp = $("#From_Plant" + index + " option:selected").text();
        if ((base_loc == "Roha" || base_loc == "Mahad" || base_loc == "Pune" || base_loc == "Sutarwadi") && fp!="Branches") {
