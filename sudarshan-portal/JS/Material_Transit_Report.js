...
function searchData() {
    ...
    Material_Transit_Report.fillSearch(str, rpp, callback_Inco);
}
...
function gotopage(objButton, str) {
    try {
        ...
        Material_Transit_Report.fillGoToPage1(strData, pgno, str, callback_Inco);
    }
    catch (exception) {
...
function setSelectedNote(req_note) {
    try {
        Material_Transit_Report.GetCurrentTime(req_note, OnSuccess);
    }
    catch (exception) {
...
function downloadfiles(index) {
    try {
        Material_Transit_Report.GetFileNames(index, OnSuccess1);
    }
    catch (exception) {
...
function viewVendor(vcode) {
    try {
        ...
        Material_Transit_Report.viewVendor(venCode, OnVendor);
    }
    catch (exception) {
