...
function getSelectDate(fromDate, toDate) {
    try {
        if (fromDate != "" && toDate != "") {
            ...
            Domestic_Travel_Request_Mode_Change.get_Journey_Data(fromDate, toDate, wiid, AjaxSucceeded);
        }
    }
    catch (ex) {
        // $('#divIns').html("");
...
function get_exp_data(index) {
    try {
        ...
        Domestic_Travel_Request_Mode_Change.get_Expense_Data(desg, j_val, travel_mode_id, travel_class_id, fplant_id, tplant_id, pk_city, index,division, ExpenseSucceeded);
    }
    catch (ex) {
        alert(ex);
    }
...
function get_dev_policy(index) {
    try {
        ...
        Domestic_Travel_Request_Mode_Change.get_dev_policy(desg, j_val, travel_mode_id, travel_class_id, DeviationSucceeded);
    }
    catch (ex) {
        alert(ex);
    }
...
function prepare_data() {
    try {
        if ($("#empno").html() == "") {
            ...
            var voucher_amount = $("#spn_voucher_amt").html();
            var adv_amount = $("#advance_amount").val();

            
            if ((parseFloat(adv_amount) > voucher_amount) && $("#ddl_Payment_Mode option:selected").text().toUpperCase() == "BANK") {
...
function change_flag(exp, index, i) {
    try {
        ...
        Domestic_Travel_Request_Mode_Change.chagable_or_not("Domestic Travel Expenses", fk_id, data, changeText);
    }
    catch (ex) {
        alert(ex);
    }
...
function chk_class_To(i) {
    ...
    if ($("#To_City" + i + " option:selected").text() == "Other") {
        ...
        Domestic_Travel_Request_Mode_Change.get_Travel_Class(pk_city, i, ClassSucceeded);
    }
}
...
function get_travel_class(rno) {
    ...
    Domestic_Travel_Request_Mode_Change.get_travel_class(Travel_Mode, rno, OnTravelClass);
}
...
function show_record() {
    ...
    var desg = $("#txt_designation").val();
    var pgno = $("pageno").val();
    Domestic_Travel_Request_Mode_Change.showall(search_data, 1, ddl, desg, division, OnshowAll);
}
...
function gotopage(objButton, str) {
    try {
        ...
        var desg = $("#txt_designation").val();
        Domestic_Travel_Request_Mode_Change.showall(strData, pgno, str, desg, division, OnshowAll);
    }
    catch (exception) {
