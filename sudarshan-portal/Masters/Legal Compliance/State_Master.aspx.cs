...
public partial class State_Master : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            AjaxPro.Utility.RegisterTypeForAjax(typeof(State_Master));
            try
            {
                if (!Page.IsPostBack)
                {
    ...
    private void showall()
    {
        try
        {
            ...
            DataTable DTS = (DataTable)ActionController.ExecuteAction("", "State_Master.aspx", "psearch", ref isdata, 0, "", "", 4, "");
            if (DTS.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < DTS.Rows.Count; i++)
                {
                    ...
                    str.Append("<td><a href='#CountryPanel' role='button' data-toggle='modal' Title='Update' onclick='bindData(" + (i + 1) + ")'><i class='fa fa-fw m-r-10 pull-left f-s-18 fa-edit'></i></a>  <input id='txt_pkid" + (i + 1) + "'  runat='server' Value='" + DTS.Rows[i]["PK_STATE_ID"].ToString() + "' style='display:none' /></td>");
    ...
    public string bindData(string Condition, string SerachString)
    {
        ...
        {
            string isdata = string.Empty;
            DataTable DTS = (DataTable)ActionController.ExecuteAction("", "State_Master.aspx", "psearch", ref isdata, 0, "", "", 5, SerachString);
            str.Append("<table style='width:60%' align='center' class='table table-bordered table-hover'>");
            str.Append("<thead>");
            str.Append("<tr class='grey'>");
            str.Append("<th>#</th><th>State Name</th><th>Update</th><th>Delete</th></tr></thead><tbody>");
    ...
    protected void btn_AddNew_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            try
            {
                ...
                string isSaved = (string)ActionController.ExecuteAction("", "State_Master.aspx", "pamd", ref isdata, 0, txt_State_Name.Text, created_by, 1, "");
                if (isSaved == "true")
                {
    ...
    protected void btn_Edit_onClick(object sender, EventArgs e)
    {
        ...
        else
        {
            try
            {
                string isdata = string.Empty;
                string created_by = txt_Username.Text.Trim();
                string isSaved = (string)ActionController.ExecuteAction("", "State_Master.aspx", "pamd", ref isdata, txt_PK_ID.Text, txt_Edit_State.Text, created_by, 2, "");
                if (isSaved == "true")
                {
    ...
    protected void btn_Delete_onClick(object sender, EventArgs e)
    {
        ...
        else
        {
            try
            {
                string isdata = string.Empty;
                string created_by = txt_Username.Text.Trim();
                string isSaved = (string)ActionController.ExecuteAction("", "State_Master.aspx", "pamd", ref isdata, txt_PK_ID.Text, txt_Edit_State.Text, created_by, 3, "");
                if (isSaved == "true")
                {
