...
public partial class Invoice_Details_Report : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                AjaxPro.Utility.RegisterTypeForAjax(typeof(Invoice_Details_Report));
                    if (!Page.IsPostBack)
                    {
    ...
    protected void submit(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            DataTable dt = (DataTable)ActionController.ExecuteAction("", "Invoice_Details_Report.aspx", "getinvoicedetails", ref isdata,txt_Username.Text, ddlStatus.SelectedValue, txt_form_Date.Text, text_To_Date.Text);
            
            if (dt != null && dt.Rows.Count > 0)
            {

                str.Append("<table id='tble' class='table table-bordered'><thead><tr class='grey'><th>#</th><th>Sr. No.</th><th>PO Number</th><th>PO Date</th><th>Cum Inv Amt</th><th>PO GV</th></tr></thead>");
                str.Append("<tbody>");
        ...
        ScriptManager.RegisterStartupScript(this, GetType(), "", "$('#tble').dataTable({'bSort': false});", true);
    ...
    public string fillDetail(int id, string username, string po, string status)
    {
        ...
        try
        {
            ...
            dtPO = (DataTable)ActionController.ExecuteAction("", "Invoice_Details_Report.aspx", "getinvoicedetailspo", ref isdata, username, po, status);
            Session["dtData"] = dtPO;
            if (dtPO != null)
            {
    ...
    protected void btn_Export_onClick(object sender, EventArgs e)
    {
        ...
        {
            ExportToExcel(str, "Invoice Detail Report");
        }
    }
    ...
    protected void ExportToExcel(StringBuilder dgview, string filename)
    {
        try
        {
            ...
            DataTable DTS = (DataTable)ActionController.ExecuteAction("", "Invoice_Details_Report.aspx", "getinvoicedetails", ref isdata, txt_Username.Text, ddlStatus.SelectedValue, txt_form_Date.Text, text_To_Date.Text);
            if (DTS != null && DTS.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < DTS.Rows.Count; i++)
                {
                    ...
                    DataTable dt = (DataTable)ActionController.ExecuteAction("", "Invoice_Details_Report.aspx", "getinvoicedetailspo", ref isdata1, txt_Username.Text, Convert.ToString(DTS.Rows[i]["Po_Number"]), ddlStatus.SelectedValue);
                    if (dt != null && dt.Rows.Count > 0)
                    {
