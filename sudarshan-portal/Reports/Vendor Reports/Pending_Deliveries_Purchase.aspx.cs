...
public partial class Pending_Deliveries_Purchase : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                AjaxPro.Utility.RegisterTypeForAjax(typeof(Pending_Deliveries_Purchase));
                if (!Page.IsPostBack)
                {
    ...
    protected string getUserData(string name, int pageno, int rpp)
    {
        ...
        try
        {
            ...
            html.Append("<tbody>");
            DataTable dtData = (DataTable)ActionController.ExecuteAction("", "Pending_Deliveries_Purchase.aspx", "getpendingdeliveries", ref data, "", name);
            if (dtData != null)
            {
                int ddl = rpp;
